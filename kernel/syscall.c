...
argraw(int n)
{
  ...
  switch (n) {
  case 0:
    return (p->trapframe+t_index)->a0;
  case 1:
    return (p->trapframe+t_index)->a1;
  case 2:
    return (p->trapframe+t_index)->a2;
  case 3:
    return (p->trapframe+t_index)->a3;
  case 4:
    return (p->trapframe+t_index)->a4;
  case 5:
    return (p->trapframe+t_index)->a5;
  }
  panic("argraw");
  return -1;
}
...
syscall(void)
{
  ...
  num = (p->trapframe+t_index)->a7;
  if(num > 0 && num < NELEM(syscalls) && syscalls[num]) {
    (p->trapframe+t_index)->a0 = syscalls[num]();
  } else {
    printf("%d %s: unknown sys call %d\n",
            p->pid, p->name, num);
    (p->trapframe+t_index)->a0 = -1;
  }
}
